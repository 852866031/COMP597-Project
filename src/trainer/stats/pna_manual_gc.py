...
"""
PNAManualGCStats — per-step timing for the manual-GC PNA trainer.

Used with the PNA measurement trainer (--trainer pna) which suppresses
automatic GC and forces gen-2 collections between epochs. Records only
the CUDA-synchronised wall-clock duration of each training step.

One CSV is written to output_dir/manual/ at the end of training:

  pna_manual_gc_bs<N>_wk<M>.csv
...
def construct_trainer_stats(conf: config.Config, **kwargs) -> base.TrainerStats:
    ...
    if device is None:
        logger.warning("PNAManualGCStats: no device provided; falling back to default")
...
class PNAManualGCStats(base.TrainerStats):
    """Minimal per-step timing for the manual-GC trainer. Records only step_ms."""
    ...
    def __init__(
        ...
        self._csv_path = os.path.join(manual_dir, f"{stem}.csv")
        ...
        logger.info("PNAManualGCStats: CSV -> %s", os.path.abspath(self._csv_path))
    ...
    def log_stats(self) -> None:
        if not self._rows:
            logger.warning("PNAManualGCStats: no rows; skipping CSV")
            return
        ...
        logger.info("PNAManualGCStats: wrote %d rows to %s",
