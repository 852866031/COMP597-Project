...
One combined overview figure plus ten individual plots are produced per batch-size:

  overview.png  — 5 × 2 grid: each row pairs energy (left) and carbon (right):
      row 1  total energy per step            | total CO₂ per step
      row 2  energy breakdown by substep      | CO₂ breakdown by substep
      row 3  energy substep pancake           | CO₂ substep pancake
      row 4  energy breakdown by hardware     | CO₂ breakdown by hardware
      row 5  energy hardware pancake          | CO₂ hardware pancake
...
# ---------------------------------------------------------------------------
# Combined overview — 5 × 2 grid
# ---------------------------------------------------------------------------

def plot_overview(step_df: pd.DataFrame, substep_df: Optional[pd.DataFrame],
                  out_path: Path, meta: dict) -> None:
    """5 × 2 combined figure.

    Layout (energy on the left, carbon on the right)::

        row 0: total energy             | total CO₂
        row 1: substep stacked area     | substep stacked area (CO₂)
        row 2: substep pancake          | substep pancake (CO₂)
        row 3: hardware stacked area    | hardware stacked area (CO₂)
        row 4: hardware pancake         | hardware pancake (CO₂)
    """
    height_ratios = [2, 2, 2.8, 2, 2.8]
    fig, axes = plt.subplots(
        5, 2,
        figsize=(18, 34),
        gridspec_kw={"height_ratios": height_ratios, "hspace": 0.55, "wspace": 0.30},
    )
    ...
    bs = meta["batch_size"]
    fig.suptitle(
        f"PNA Carbon — Energy & Emissions Overview  ·  batch size {bs}",
    ...
    # ------------------------------------------------------------------ #
    ax = axes[0, 0]
    ax.bar(x, total_energy * KWH_TO_MWH, width=1.0, color="#457B9D", alpha=0.80)
    _draw_epoch_vlines(ax, epoch_starts)
    if epoch_starts:
        ax.legend(["Epoch start"], fontsize=8, loc="upper right")
    ax.set_title("Total Energy per Step", fontsize=11, fontweight="bold")
    ax.set_xlabel("Step")
    ax.set_ylabel("Energy (mWh)")
    ax.yaxis.grid(True, linestyle="--", alpha=0.4)
    ax.set_axisbelow(True)
    ...
    ax = axes[0, 1]
    ax.bar(x, total_emit * KG_TO_UG, width=1.0, color="#E63946", alpha=0.80)
    _draw_epoch_vlines(ax, epoch_starts)
    ax.set_title("Total CO₂ Emissions per Step", fontsize=11, fontweight="bold")
    ax.set_xlabel("Step")
    ax.set_ylabel("CO₂ (µg CO₂eq)")
    ax.yaxis.grid(True, linestyle="--", alpha=0.4)
    ax.set_axisbelow(True)

    # ------------------------------------------------------------------ #
    # Row 1 — substep stacked areas
    # ------------------------------------------------------------------ #
    if substep_df is not None:
        pivot_e = (
            ...
            .pivot_table(index="step_idx", columns="phase",
                         values="energy_consumed", aggfunc="sum")
            .reindex(columns=PHASE_ORDER).fillna(0.0)
        )
        pivot_c = (
            substep_df
            .pivot_table(index="step_idx", columns="phase",
        ...
        base = step_df[["global_step", "step_idx"]]
        merged_e = (base.merge(pivot_e.reset_index(), on="step_idx", how="left")
                    .sort_values("global_step").reset_index(drop=True))
        merged_c = (base.merge(pivot_c.reset_index(), on="step_idx", how="left")
                    .sort_values("global_step").reset_index(drop=True))

        for col_idx, (merged, unit_factor, ylabel, title) in enumerate([
            (merged_e, KWH_TO_MWH, "Energy (mWh)",    "Energy Breakdown by Substep"),
            (merged_c, KG_TO_UG,   "CO₂ (µg CO₂eq)", "CO₂ Breakdown by Substep"),
        ]):
            ax = axes[1, col_idx]
            xx  = merged["global_step"].to_numpy()
            bot = np.zeros(len(merged))
            for phase in PHASE_ORDER:
                if phase not in merged.columns:
                    continue
                vals = merged[phase].to_numpy() * unit_factor
                ax.fill_between(xx, bot, bot + vals,
                                alpha=0.80, color=PHASE_COLORS[phase],
                                label=PHASE_LABELS[phase])
                bot += vals
            _draw_epoch_vlines(ax, epoch_starts)
        ...
        for j in range(2):
            axes[1, j].text(0.5, 0.5, "no substep data available",
                            ha="center", va="center",
                            transform=axes[1, j].transAxes, fontsize=11)
            axes[1, j].set_axis_off()

    # ------------------------------------------------------------------ #
    # Row 2 — substep pancakes
    # ------------------------------------------------------------------ #
    if substep_df is not None:
        for col_idx, (val_col, unit_factor, unit_str, fmt, title) in enumerate([
            ("energy_consumed", KWH_TO_MWH, "mWh", ".3f", "Energy Share by Substep"),
            ("emissions",       KG_TO_UG,   "µg",  ".2f", "CO₂ Share by Substep"),
        ]):
            ax = axes[2, col_idx]
            means = {p: substep_df.loc[substep_df["phase"] == p, val_col].mean()
                     for p in PHASE_ORDER}
            vals   = [means.get(p, 0.0) * unit_factor for p in PHASE_ORDER]
            labels = [PHASE_LABELS[p] for p in PHASE_ORDER]
            colors = [PHASE_COLORS[p] for p in PHASE_ORDER]
            total  = sum(vals)
            center = f"mean\n{total:{fmt}} {unit_str}"
            result = _draw_pancake(ax, vals, labels, colors, center)
            ax.set_title(title, fontsize=11, fontweight="bold")
            if result:
                wedges, lbls, pvals = result
                ax.legend(
                    wedges,
                    [f"{l}  ({v:{fmt}} {unit_str})" for l, v in zip(lbls, pvals)],
                    loc="lower center", bbox_to_anchor=(0.5, -0.14),
                    ncol=2, fontsize=8, frameon=False,
                )
    else:
        for j in range(2):
            axes[2, j].text(0.5, 0.5, "no substep data available",
                            ha="center", va="center",
                            transform=axes[2, j].transAxes, fontsize=11)
            axes[2, j].set_axis_off()

    # ------------------------------------------------------------------ #
    # Row 3 — hardware stacked areas
    # ------------------------------------------------------------------ #
    ax_e = axes[3, 0]
    ax_c = axes[3, 1]
    bot_e = np.zeros(len(step_df))
    bot_c = np.zeros(len(step_df))
    ...
    for hw in HW_ORDER:
        ...
        vals_e = hw_energy * KWH_TO_MWH
        ax_e.fill_between(x, bot_e, bot_e + vals_e,
                          alpha=0.80, color=HW_COLORS[hw], label=HW_LABELS[hw])
        bot_e += vals_e

        hw_emit = np.where(np.isfinite(safe_energy),
                           total_emit * (hw_energy / safe_energy), 0.0) * KG_TO_UG
        ax_c.fill_between(x, bot_c, bot_c + hw_emit,
                          alpha=0.80, color=HW_COLORS[hw], label=HW_LABELS[hw])
        bot_c += hw_emit

    for ax, title, ylabel in [
        (ax_e, "Energy Breakdown by Hardware",  "Energy (mWh)"),
        (ax_c, "CO₂ Breakdown by Hardware",     "CO₂ (µg CO₂eq)"),
    ]:
        _draw_epoch_vlines(ax, epoch_starts)
        ax.set_title(title, fontsize=11, fontweight="bold")
        ax.set_xlabel("Step")
    ...
    for hw in HW_ORDER:
        ...
        if col not in step_df.columns:
            continue
        hw_energy = step_df[col].to_numpy()
        hw_lbls.append(HW_LABELS[hw])
        hw_cols.append(HW_COLORS[hw])
        hw_vals_e.append(float(step_df[col].mean()) * KWH_TO_MWH)
        hw_vals_c.append(float(np.nanmean(
            np.where(np.isfinite(safe_energy),
                     total_emit * (hw_energy / safe_energy), 0.0)
        )) * KG_TO_UG)

    for col_idx, (vals, unit_str, fmt, title) in enumerate([
        (hw_vals_e, "mWh", ".3f", "Energy Share by Hardware"),
        (hw_vals_c, "µg",  ".2f", "CO₂ Share by Hardware"),
    ]):
        ax = axes[4, col_idx]
        total  = sum(vals)
        center = f"mean\n{total:{fmt}} {unit_str}"
        result = _draw_pancake(ax, vals, hw_lbls, hw_cols, center)
        ax.set_title(title, fontsize=11, fontweight="bold")
        if result:
            wedges, lbls, pvals = result
            ax.legend(
...
def process_file(step_csv: Path) -> None:
    ...
    # --- Combined overview (5 × 2 grid) ---
    plot_overview(step_df, substep_df, out / f"{stem}_overview.png", meta)
