...
Usage
-----
# Auto-discover carbon/pna_carbon_bs512_wk*_steps.csv in the same directory:
    python pna_result/plot_carbon.py

# Point at a specific steps CSV:
    python pna_result/plot_carbon.py --step pna_result/carbon/pna_carbon_bs512_wk0_steps.csv
...
def discover_step_csvs(directory: Path) -> List[Path]:
    """Find pna_carbon_bs512_wk*_steps.csv in carbon/ subdir."""
    carbon_dir = directory / "carbon"
    if not carbon_dir.is_dir():
        return []
    return sorted(carbon_dir.glob("pna_carbon_bs512_wk*_steps.csv"))
...
def main() -> None:
    ...
    if args.step:
        ...
        if not csv_files:
            print(
                f"No pna_carbon_bs512_wk*_steps.csv found in {script_dir / 'carbon'}.\n"
