#!/usr/bin/env python3
"""
plot_batchsize.py — compare PNA metrics across batch sizes 256, 512, 1024, and 4096.
...
------
  ...
  3. bs_energy_total.png    — mean per-step avg total energy (mWh), per batch size
...
BATCH_SIZES: List[int] = [256, 512, 1024, 4096]
...
def check_files(result_dir: Path) -> None:
    ...
    if missing:
        ...
        print("Run the start-pna-*.sh scripts with -bs 256, -bs 512, -bs 1024, and -bs 4096 first.\n")
        for p in missing:
            print(f"  {p}")
        sys.exit(1)
...
def plot_energy_total(step_data: Dict[int, pd.DataFrame], out_path: Path) -> None:
    ...
    fig, ax = plt.subplots(figsize=(7, 5))
    fig.suptitle(
        "PNA — Per-Step Avg Total Energy vs Batch Size\n"
        "mean over all measurement windows · CodeCarbon (mWh)",
        fontsize=13, fontweight="bold",
    )
