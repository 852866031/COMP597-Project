...
def discover_gc_on_csv(directory: Path) -> Optional[Path]:
    """Find pna_spike_bs*_gc_on.csv in spike/ subdir."""
    spike_dir = directory / "spike"
    if not spike_dir.is_dir():
        return None
    matches = sorted(spike_dir.glob("pna_spike_bs*_gc_on.csv"))
...
def main() -> None:
    ...
    if gc_on_path is None:
        print("No pna_spike_bs*_gc_on.csv found in spike/. Run start-pna-spike.sh first.")
        sys.exit(0)
